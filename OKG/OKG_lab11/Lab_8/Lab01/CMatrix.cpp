...
double CMatrix::operator()(int i, int j) const {
	// Проверки границ, как в вашей не-const версии
	if ((i >= n_rows) || (j >= n_cols) || i < 0 || j < 0) { // Добавил i < 0 || j < 0
		TCHAR* error = _T("CMatrix::operator()(const): выход индекса за границу диапазона ");
		MessageBox(NULL, error, _T("Ошибка"), MB_ICONSTOP);
		exit(1); // или throw exception
	}
	return array[i][j];
}
