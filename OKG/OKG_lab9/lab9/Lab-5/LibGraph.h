...
CMatrix Normalize(CMatrix& V)
{
	CMatrix Res(V); // Создаем копию
	double mod = ModVec(Res);
	if (mod < 1e-9) // Избегаем деления на ноль или очень маленькие числа
	{
		// Опционально: обработать случай нулевого вектора, например, вернуть V или установить ошибку
		return Res; // Вернуть оригинал, если модуль слишком мал
	}
	Res(0) = Res(0) / mod;
	Res(1) = Res(1) / mod;
	Res(2) = Res(2) / mod;
