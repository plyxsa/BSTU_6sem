...
//-----------------------------------------------------------------------------
void CRectD::DrawCircle(CDC* pDC, double centerX, double centerY, double radius, COLORREF color)
{
	// Создаем перо для рисования окружности
	CPen pen(PS_SOLID, 2, color); // Толщина пера 2, цвет задается параметром color
	CPen* pOldPen = pDC->SelectObject(&pen);

	// Вычисляем координаты ограничивающего прямоугольника для окружности
