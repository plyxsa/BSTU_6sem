...
spool "SQL_MIGRATION_&filename..log"

-- Password file execution
@passworddefinition.sql

PROMPT Dropping Role ROLE_SQL_MIGRATION ...
DROP ROLE ROLE_SQL_MIGRATION ;
PROMPT Creating Role ROLE_SQL_MIGRATION ...
CREATE ROLE ROLE_SQL_MIGRATION ;

-- PROMPT Drop University user
-- drop user University cascade;
   
PROMPT Create user University
CREATE USER University IDENTIFIED BY &&University_password PASSWORD EXPIRE ACCOUNT LOCK /* DEFAULT TABLESPACE USERS  TEMPORARY TABLESPACE TEMP */;
GRANT CREATE SESSION, RESOURCE, CREATE VIEW, CREATE MATERIALIZED VIEW, CREATE SYNONYM, UNLIMITED TABLESPACE TO University;
...
GRANT ALL ON utils TO ROLE_SQL_MIGRATION;
grant ALL on utils to University with grant option;

set define on
prompt connecting to University
alter session set current_schema=University;
set define off
-- DROP TABLE ContactInfo CASCADE CONSTRAINTS;


PROMPT Creating Table ContactInfo ...
CREATE TABLE ContactInfo (
  ContactID NUMBER(10,0) NOT NULL,
  PhoneNumber RAW(8000)
);


PROMPT Creating Primary Key Constraint PK__ContactI__5C6625BB9D4BA36A on table ContactInfo ... 
ALTER TABLE ContactInfo
ADD CONSTRAINT PK__ContactI__5C6625BB9D4BA36A PRIMARY KEY
(
  ContactID
)
ENABLE
...
GRANT ALL ON ContactInfo TO ROLE_SQL_MIGRATION;
